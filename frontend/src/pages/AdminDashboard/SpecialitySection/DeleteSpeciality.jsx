...
const DeleteSpeciality = ({specialityID, onClose, refreshSpecialities }) => {
    ...
    const handleDelete = async () => 
    {
        ...
        const response = await axios.delete(`${backendUrl}/api/admin/delete-speciality/${specialityID}`);
        console.log("Răspuns backend la ștergere:", response);
        if (response.status === 200 && response.data?.success) {
            localStorage.setItem("refreshSpeciality", "true");
            toast.success(response.data.message || "Specialitatea a fost ștearsă!");
            refreshSpecialities();
            onClose();
            return;
        }else {
            toast.error(response.data.message || "Eroare la ștergerea specialității.");
        }
        } catch (err) {
     ...
     <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-white p-6 rounded-lg shadow-xl w-[90%] max-w-md text-center">
        ...
        <p className="text-gray-600 mb-6">
          ...
          <br />
          <span className="font-medium text-red-600">{specialityID}</span>
        </p>
