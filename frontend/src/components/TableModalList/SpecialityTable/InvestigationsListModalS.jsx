...
import { faClose, faMicroscope, faStopwatch, faStar } from '@fortawesome/free-solid-svg-icons';
import Loader from '../../Loader';

const InvestigationListModalS = ({ isOpen, onClose, specialityID, specialityName }) => {
  ...
  const fetchInvestigations = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/api/admin/get-investigations-by-speciality`, { specialityID });
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center overflow-y-auto">
      <div className="bg-white p-6 rounded-lg shadow-xl w-[90%] max-w-3xl max-h-[90%] overflow-y-auto relative">
        ...
        <h2 className="text-2xl font-bold text-center mb-1 text-gray-800">
          Investigațiile pentru Specialitatea
        </h2>
            ...
            {investigations.map((inv) => {
              ...
              return (
                <li
                  ...
                  className={`flex items-start gap-3 p-4 rounded-lg shadow-sm transition-all duration-200 hover:scale-103 hover:shadow-md border-l-4 bg-pink-50 border-pink-600`}
                >
                  <div className="flex flex-col items-center mt-3 mr-1">
                    <FontAwesomeIcon icon={faMicroscope} className="text-purple-700 text-2xl mt" />
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 italic">
                      Durată: {inv.duration} min <FontAwesomeIcon icon={faStopwatch} className="ml-1"/> {inv.status}
                    </p>
                  </div>
                </li>
              );
...
export default InvestigationListModalS;
